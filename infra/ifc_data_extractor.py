...
def _entity_name(entity: Any) -> str | None:
    for attr in ("Name", "LayerSetName", "ProfileSetName"):
    ...
    for unit in raw_units:
        ...
        if conversion_factor is not None:
            ...
            record["conversion_factor"] = {
                ...
                "unit": _entity_name(unit_component) or _entity_class(unit_component),
            }
...
def _element_identity(element: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "ifc_class": _entity_class(element),
        ...
        "type": (
            ...
            if type_entity is None
                ...
                "id": _entity_id(type_entity),
                "global_id": getattr(type_entity, "GlobalId", None),
                "ifc_class": _entity_class(type_entity),
        ...
        "container": (
            ...
            if container is None
                ...
                "id": _entity_id(container),
                "global_id": getattr(container, "GlobalId", None),
                "ifc_class": _entity_class(container),
...
def _flatten_psets(
    ...
    *,
    ...
    for set_name in sorted(psets):
        ...
        for property_name in sorted(key for key in values if key != "id"):
            rows.append(
                {
                    **identity,
...
def _material_rows(element: Any) -> list[dict[str, Any]]:
    ...
    for material in ifc_element.get_materials(element, should_inherit=True):
        material_id = _entity_id(material)
        material_class = _entity_class(material)
        material_name = _entity_name(material)
        key = (material_id, material_class, material_name)
...
class IfcOpenShellDataExtractor(DataExtractor):
    """Extract structured IFC metadata without performing geometry conversion."""
    ...
    def inspect(
        ...
        *,
        ...
        printer.status("EXTRACT", "Inspecting IFC source", "info")
        ...
        with tempfile.TemporaryDirectory(prefix="bimap-ifc-extract-") as directory_name:
    ...
    def extract(
        ...
        *,
        ...
        printer.status("EXTRACT", "Extracting IFC model data", "info")
        source = ExtractionSourceFormat.parse(source_format)
        ...
        with tempfile.TemporaryDirectory(prefix="bimap-ifc-extract-") as directory_name:
            ...
            if ExtractionDataset.ELEMENTS in selected:
                rows = tuple(
                    _json_value(_element_identity(product))
                    for product in products
                )
            ...
            if ExtractionDataset.PROPERTIES in selected:
                ...
                for product in products:
                    property_rows.extend(
                        _flatten_psets(product, quantities=False)
                    )
                rows = tuple(_json_value(row) for row in property_rows)
            ...
            if ExtractionDataset.QUANTITIES in selected:
                ...
                for product in products:
                    quantity_rows.extend(
                        _flatten_psets(product, quantities=True)
                    )
                rows = tuple(_json_value(row) for row in quantity_rows)
            ...
            if ExtractionDataset.MATERIALS in selected:
                ...
                for product in products:
                    material_rows.extend(_material_rows(product))
                rows = tuple(_json_value(row) for row in material_rows)
            ...
            return ExtractedModelData(
                ...
                project=_json_value(_project_record(model)),
